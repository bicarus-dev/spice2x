...
namespace launcher::splash {
    ...
    namespace {
        ...
        void CALLBACK window_event_proc(
            ...
            if (event == EVENT_OBJECT_SHOW && object_id == OBJID_WINDOW &&
                ...
                close("WinEvent EVENT_OBJECT_SHOW");
            }
        }
        ...
        void run_ui_thread() {
            ...
            // observe windows becoming visible in this process. the first one closes the splash
            // without requiring renderer-specific integration.
    ...
    void close(const char *source) {
        HWND window = splash_window.load();
        if (!window || close_requested.exchange(true)) {
            return;
        }

        log_info("splash", "close requested by {}", source);
        if (!PostMessageW(window, WM_CLOSE, 0, 0)) {
            const DWORD error = GetLastError();
            close_requested.store(false);
            log_warning("splash", "failed to request close: {}", error);
        }
    }
